mod_home_ui <- function(id){
  ...
  tagList(
    fluidRow(
      tags$h2(tags$strong("IMPC Disease Models Portal"),
              style = "font-size:40px;",
              align = "center", 
              id = ns("page-title"))),
      fluidRow(
      box(width = 12, solidHeader = TRUE, status = "primary",
      tags$p(tags$h4(tags$strong(("About")), 
                    style = "font-size:25px;",
                     align = "left",
                     id = "page-text")),
      tags$p(tags$h4(style = "font-size:20px;", style = "line-height:1.5",
      ...
      PhenoDigm algorithm.", align = "left", id = "page-text")),
      tags$p(tags$h4(style = "font-size:20px;", style = "line-height:1.5",
                     ...
                     tags$a(href = "https://www.mousephenotype.org/", 
                            "IMPC portal"),".The human phenotype annotations 
                     corrsponding to ",tags$a(href = "https://www.omim.org/", "OMIM"), 
                     "and",tags$a(href = "https://www.orpha.net/", "Orphanet"),
                     "disorders used in the pipeline were obtained from the",
                     tags$a(href = "https://hpo.jax.org", "Human Phenotype Ontology (HPO)"),
                     "resource.", align = "left", id = "page-text")),
      tags$p(tags$h4(style = "font-size:20px;", style = "line-height:1.5",
                     ...
                     phenotypic overlap.", align = "left", id = "page-text")),
      tags$p(tags$h4(style = "font-size:20px;", style = "line-height:1.5",
    ...
    box(width = 12, solidHeader = TRUE, status = "primary",
        fluidRow(
          column(8,
    tags$p(tags$h4(tags$strong(("Gene summary")), 
                   style = "font-size:25px;",
                   align = "left",
                   id = "page-text")),
    ...
    title = "Search Gene",
    status = "warning",
    textInput(inputId = ns("gene_search"), 
              label = NULL, 
              placeholder = "SPATA16",
              width = 200))),
  ...
  fluidRow(
    tags$p(tags$h4("Last updated 07.05.2024",
...
mod_home_server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    gene_info <- load_gene_summary() %>%
      ...
      select(gene_symbol,disease_gene, IMPC_pipeline, IMPC_phenotypes,
             PhenoDigm_match, max_score,disorder_name) %>%
    ...
    model_info <- load_phenodigm()
      ...
      model_info_summary = model_info %>%
        ...
        summarise(disorder_name = paste0(unique(disorder_name),
                                         collapse = "|"),
                  score = paste0(unique(score),
                                         collapse = "|"),
                  query_phenotype = paste0(unique(query_phenotype),
                                         collapse = "|"),
                 match_phenotype = paste0(unique(match_phenotype),
                                         collapse = "|"))
      ...
      gene_selected <- reactive({
        ...
        gene_info_model %>%
          filter(gene_symbol %in% c(toupper(as.character(input$gene_search))))
    ...
    output$Gene_in_IMPC_Box <- renderValueBox({
      valueBox(
        value = tags$p("Gene in IMPC", 
                       style = "font-size: 50%;"),
        #ifelse(dim(gene_selected())[1] == 0, "Gene entered IMPC pipeline",
        #       "Gene has not entered IMPC pipeline"),
        ifelse(gene_selected()$IMPC_pipeline =="yes", "Gene entered IMPC pipeline",
               "Gene has not entered IMPC pipeline"),
    ...
    output$Gene_in_IMPC_Disease_Box <- renderValueBox({
      valueBox(
        value = tags$p("Gene with human disease association", 
                       style = "font-size: 50%;"),
       # ifelse(dim(gene_selected())[1] == 0, "-",
       #        gene_selected()$disorder_name),
        ifelse(gene_selected()$disease_gene =="yes", 
    ...
    output$Phenotypes_Available_Box <- renderValueBox({
      valueBox(
        value = tags$p("Associated abnormal phenotypes", 
                       style = "font-size: 50%;"),
       # ifelse(dim(gene_selected())[1] ==0, "No","Yes"),
        ifelse(gene_selected()$IMPC_phenotypes =="yes", 
    ...
    output$PhenoDigm_Score_Box <- renderValueBox({
      valueBox(
        value = tags$p("Maximum PhenoDigm score", 
                       style = "font-size: 50%;"),
       # ifelse(dim(gene_selected())[1] ==0, "-",
       #        gene_selected()$score),
        ifelse(!is.na(gene_selected()$max_score), gene_selected()$max_score,"-"),
        color = "blue"
      )
    })  
      ...
      output$Mouse_Phenotypes_Box <- renderValueBox({
        valueBox(
          value = tags$p("Matching mouse phenotypes", 
                         style = "font-size: 50%;"),
          #ifelse(dim(gene_selected())[1] ==0, "-",
           #      gene_selected()$match_phenotype),
          ifelse(!is.na(gene_selected()$match_phenotype), 
                 gene_selected()$match_phenotype,"-"),
      ...
      output$Human_Phenotypes_Box <- renderValueBox({
        valueBox(
          value = tags$p("Matching disorder phenotypes", 
                         style = "font-size: 50%;"),
          #ifelse(dim(gene_selected())[1] ==0, "-",
          #       gene_selected()$query_phenotype),
          ifelse(!is.na(gene_selected()$query_phenotype), 
                 gene_selected()$query_phenotype,"-"),
                        color = "blue"
                 )
      })   
