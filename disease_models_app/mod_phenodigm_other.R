  ...
  tags$div(
    fluidRow(
      ...
      tags$p(tags$h4(style = "font-size:20px;",
                     ...
                     known disorders. Only mouse model - disease paiars with a PhenoDigm score > 40
...
mod_phenodigm_other_ui <- function(id){
  ...
  tagList(
    ...
    intro(),
    tags$br(),
    fluidRow(DT::dataTableOutput(ns("phenodigm_other_table"))),
...
mod_phenodigm_other_server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    output$phenodigm_other_table <- DT::renderDataTable({
